...
def _load_model():
    ...
    try:
        ...
        logger.info(
            'TFLite model loaded via ai-edge-litert. '
            'Output shape: %s',
            out_shape)
        ...
        try:
            ...
            _model_loaded = True
            out_shape = _interpreter.get_output_details()[0]['shape']
            logger.info('TFLite model loaded via tensorflow.'
                        'Output shape: %s', out_shape)
...
def _preprocess_image(image_path: str) -> np.ndarray:
    ...
    """
    img = Image.open(
        image_path).convert('RGB').resize(IMAGE_SIZE, Image.LANCZOS)
...
def predict_image(image_path: str) -> dict:
    ...
    risk = RISK_CONFIG.get(
        predicted_class, {'level': 'Unknown', 'color': 'secondary'})
...
def load_recommendation(predicted_class: str, lang: str = 'en') -> dict:
    ...
    if not file_path.exists():
        logger.warning(
            'Recommendations file missing for'
    ...
    try:
        ...
        if not rec:
            logger.warning('No recommendation for class=%s lang=%s',
