...
def export_csv(modeladmin, request, queryset):
    ...
    writer.writerow(['ID', 'Predicted Class', 'Display Name',
                     'Confidence (%)', 'Risk Level', 'Date'])
    for p in queryset:
        writer.writerow([p.id, p.predicted_class, p.display_name,
                         f'{p.confidence:.2f}',
                         p.risk_level, p.created_at.strftime(
                             '%Y-%m-%d %H:%M')])
    return response
...
class PredictionAdmin(admin.ModelAdmin):
    list_display = ['id', 'image_preview', 'user', 'display_name',
                    'confidence_display', 'risk_badge', 'language_used', 'created_at']
    list_filter = ['predicted_class', 'risk_level', 'language_used', 'created_at']
    search_fields = ['predicted_class', 'display_name', 'notes', 'user__username']
